...
import { Scan, CheckCircle, AlertTriangle, Printer, Box, ArrowLeft, Lock, Info, PlayCircle, PlusSquare, ArrowRight, GitMerge, ChevronRight, X, RefreshCw, FileDown, Layers, LogOut, CheckSquare, Square, List, Hash } from 'lucide-react';
...
function StationInterface({ operation, route, onBack, user }: StationProps) {
  ...
  const handleFinishSetup = async (existingOrder?: WorkOrder, existingPart?: PartNumber) => {
      ...
      if (targetPart) {
          ...
          if (targetPart.processRouteId !== route.id) {
           ...
           // Failing early is better UX.
           // However, if parts list is not synced, this might be risky. Assuming db.getParts() is fresh.
      }
          ...
          if (res.success) {
              ...
              if (order) {
                  ...
                  if (part) {
                      ...
                      // Note: PCB_SERIAL printing at setup is disabled (handled at scan time per unit)
                      
                      try {
                          if (isAccessories) {
                              // ACCESORIOS: Imprimir cantidad exacta de la orden (sin exclusiones, imprime lo configurado)
                              ...
                              // LOT BASED: Imprimir etiquetas de CAJA (Excluyendo Nameplate)
                              // Las Nameplates se imprimen en Empaque (Final)
                              showLoading("Imprimiendo etiquetas de caja...");
                              await db.printLabel(res.orderNumber, part.partNumber, { 
                                  sku: part.productCode, 
                                  quantity: 1,
                                  excludeLabelTypes: ['NAMEPLATE'], 
                                  jobDescription: `Batch Labels ${res.orderNumber}`
                              });
                              setStatusMsg({ type: 'success', text: `Lote Creado + Etiquetas Enviadas` });
                          }
                      } catch (printErr: any) {
  ...
  const handleReprintSuffix = async () => {
      ...
      try {
          ...
          await db.printLabel(match.serialNumber, activeOrderPart.partNumber, {
              ...
              excludeLabelTypes: ['CARTON1', 'CARTON2'] // Only Nameplate typically for individual reprint
          });
  ...
  const handleScanTrayInitial = async (e: React.FormEvent) => {
      ...
      // Calculate remaining quantity based on Total Order vs Already Scanned/Generated
      ...
      // LOGIC: Generate remaining items, but CAP at 100 per tray.
      // E.g. Order 600, Scanned 0 -> Remaining 600 -> Generate 100
      // E.g. Order 10, Scanned 0 -> Remaining 10 -> Generate 10
      ...
      try {
          ...
          if (res.success) {
              ...
              a.href = url;
              a.download = `TRAY_${trayInput}_${activeOrder.orderNumber}_${timestamp}.csv`;
              a.click();
              window.URL.revokeObjectURL(url);
              
              setStatusMsg({ type: 'success', text: `Charola ${trayInput} generada con ${quantityToGenerate} unidades. CSV descargado.` });
              setTrayInput('');
              setTrayGenerated(true); // Show Finish button
              setTrayIsFinished(false);
  ...
  const handleScanTrayProcessing = async (e: React.FormEvent) => {
      e.preventDefault();
      if (!trayInput || !activeOrder || !activeOrderPart) return; // Must have context
      ...
      try {
          ...
          showAlert("Error", e.message, "error");
          setTrayInput(''); // Clear input on error
      } finally { hideLoading(); }
  };
  ...
  const handleProcessTrayFinal = async () => {
      ...
      try {
          ...
          if (part) {
              // EMPAQUE (TRAY): Imprimir etiquetas para cada unidad.
              // El backend automáticamente busca la configuración NAMEPLATE para cada serial.
              // Si son 10 unidades, imprime 10. Si son 100, imprime 100.
  ...
  const handleFinishAccessories = async () => {
      ...
      try {
          // IMPORTANT: Accessories don't have scanned serials, so we generate them in batch 
          // marked as complete to satisfy the "10/10" requirement on Dashboard.
          await db.generateBatchSerials({
              orderNumber: activeOrder.orderNumber,
              partNumberId: activeOrderPart.id,
              currentOperationId: operation.id,
              operatorId: user.id,
              quantity: activeOrder.quantity, // Populate the full order quantity
              autoComplete: true // Force complete status
          });
  ...
  const processInitialOp = async (serial: string) => {
    ...
    // PCB SERIAL - INITIAL SCAN: Print CARTON1 and CARTON2 automatically for this specific serial
    if (activeOrderPart.serialGenType === 'PCB_SERIAL') {
        try {
             await db.printLabel(serial, activeOrderPart.partNumber, {
                 sku: activeOrderPart.productCode,
                 quantity: 1,
                 excludeLabelTypes: ['NAMEPLATE'], // Exclude NAMEPLATE, allow CARTON1 and CARTON2
  ...
  const processFinalOp = async (serial: string) => {
    ...
    await db.saveSerial(unit);
    try {
        // PCB SERIAL EMPAQUE: Print ONLY Nameplate (Exclude Cartons/Box Labels)
        // Las etiquetas de caja se imprimieron en la estación inicial.
                    ...
                    {operation.isInitial && isOrderComplete ? (
                         <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 text-center">
                             ...
                             <div className="grid grid-cols-2 gap-3">
                                ...
                                <button onClick={() => { setActiveOrder(null); setSapOrderInput(''); setQtyInput(''); setModelInput(''); setSetupStep(1); }} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 shadow flex items-center justify-center">
                                ...
                                <div className="space-y-4">
                                    {!trayGenerated ? (
                                        ...
                                        <div className="text-center bg-green-50 p-6 rounded-xl border border-green-100">
                                            ...
                                            <p className="font-bold text-green-800 mb-4">Charola Generada e Impresa</p>
                                            <button onClick={handleFinishTrayInitial} className="w-full py-3 bg-green-600 text-white rounded-lg font-bold shadow hover:bg-green-700">
                                                Confirmar y Liberar Charola
                                            </button>
                                        </div>
                                    )}
                                </div>
...
// ... TrayProgressSummary and SerialProgressList remain unchanged ...
